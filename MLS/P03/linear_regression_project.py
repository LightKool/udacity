...
# 
# $ Ab = \begin{bmatrix}
#     1 & 0 & \frac{5}{7} & -\frac{1}{7} \\
#     0 & -1 & -\frac{34}{7} & \frac{4}{7} \\
#     0 & -7 & -\frac{29}{7} & \frac{10}{7} \end{bmatrix}$
# 
# $ --> \begin{bmatrix}
#     1 & 0 & \frac{5}{7} & -\frac{1}{7} \\
#     0 & 1 & \frac{29}{49} & -\frac{10}{49} \\
#     0 & 0 & -\frac{209}{49} & \frac{18}{49} \end{bmatrix}$
#     
# $ --> \begin{bmatrix}
#     1 & 0 & 0 & -\frac{17}{209} \\
#     0 & 1 & 0 & -\frac{32}{209} \\
#     0 & 0 & 1 & -\frac{18}{209} \end{bmatrix}$
#     
# $...$
...
# 
# $ Ab = \begin{bmatrix}
#     1 & -\frac{1}{7} & \frac{8}{7} & \frac{1}{7} \\
#     0 & 0 & 0 & 1 \\
#     0 & \frac{55}{7} & -\frac{34}{7} & \frac{8}{7} \end{bmatrix}$
# 
# $ --> \begin{bmatrix}
#     1 & 0 & \frac{58}{55} & \frac{111}{385} \\
#     0 & 1 & -\frac{34}{55} & \frac{8}{55} \\
#     0 & 0 & 0 & 1 \end{bmatrix}$
#     
# $ --> \begin{bmatrix}
#     1 & 0 & \frac{58}{55} & \frac{111}{385} \\
...
def gj_Solve(A, b, decPts=4, epsilon=1.0e-16):
    ...
    for row in range(rows):
        ...
        for row2 in range(row+1, rows):
            if abs(M[row2][row]) > abs(M[maxrow][row]):
                maxrow = row2
        swapRows(M, row, maxrow)
        if abs(M[row][row]) < epsilon:
            return None
        scaleRow(M, row, 1/M[row][row])
        for row2 in (range(row) + range(row+1,rows)):
            scale = -M[row2][row]/M[row][row]
            addScaledRow(M, row2, row, scale)
    result = [[x] for x in [r[cols-1] for r in M]]
    return result
...
def linearRegression2D(X,Y):
    x = [list(t) for t in zip(X, [1 for x in X])]
    xT = transpose(x)
    y = [[y] for y in Y]
    A = matxMultiply(xT, x)
    b = matxMultiply(xT, y)
    return (r[0] for r in gj_Solve(A, b))
...
def linearRegression(X,Y):
    x = [l+[v] for l,v in zip(X, [1 for x in X])]
    xT = transpose(x)
    y = [[y] for y in Y]
    A = matxMultiply(xT, x)
    b = matxMultiply(xT, y)
    return [x[0] for x in gj_Solve(A, b)]
